  ...
  return onError(({ graphQLErrors, networkError, operation: _operation, forward: _forward }) => {
    ...
    if (graphQLErrors) {
      graphQLErrors.forEach(({ message, locations, path, extensions }) => {
        ...
        if (
          ...
          authStore.setError(new Error('Sesión expirada. Por favor inicia sesión nuevamente.'))
        ...
        if (
          ...
          authStore.setError(new Error('No tienes permisos para realizar esta acción.'))
    ...
    if (networkError) {
      ...
      if ('statusCode' in networkError) {
        ...
        switch (statusCode) {
          case 401:
            authStore.setError(new Error('Credenciales inválidas. Por favor inicia sesión nuevamente.'))
            authStore.clearAuth()
          ...
          case 429:
            authStore.setError(new Error('Demasiadas solicitudes. Por favor intenta más tarde.'))
            break
          ...
          case 503:
          case 504:
            authStore.setError(new Error('Error del servidor. Por favor intenta más tarde.'))
            break

          default:
            authStore.setError(new Error('Error de conexión. Verifica tu conexión a internet.'))
        }
      } else {
        authStore.setError(new Error('Error de red. Verifica tu conexión a internet.'))
      }
    }
  })
}
  ...
  return setContext(async (_, { headers }) => {
    ...
    if (isAuthenticated.value) {
      try {
        ...
        const authStore = useAuthStore()
        authStore.setError(new Error('Tu sesión ha expirado. Por favor inicia sesión nuevamente.'))
        authStore.clearAuth()
